...
def running(x, y, _gui):
    ...
    if frame_id < 96:
        evolve(x, y)
        fill_img(base_size, x)
    elif 96 <= frame_id < 576:
        offset = (frame_id - 96) * (n - base_size) / 480
        evolve(x, y)
...
def main():
    ...
    init_buffer, init_width, init_height = rle_to_numpy('quant_sim_meta.rle')
